...
Rails.application.config.sorcery.configure do |config|
  ...
  # Default: `[]`
  #
  config.external_providers = %i[auth0]
  ...
  #
  config.auth0.key = ENV['AUTH0_CLIENT_ID']
  config.auth0.secret = ENV['AUTH0_CLIENT_SECRET']
  config.auth0.callback_url = "http://localhost:3000/oauth/callback?provider=auth0"
  config.auth0.site = "https://#{ENV['AUTH0_DOMAIN']}"
  config.auth0.user_info_mapping = {:email => "email" }
